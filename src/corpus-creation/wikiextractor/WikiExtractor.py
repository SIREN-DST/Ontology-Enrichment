...
options = SimpleNamespace(
    ...
    ##
    # Path to doc2vec model used for similarity comparison
    doc2vec = '',
    ...
    ##
    # Similarity threshold of doc2vec model used for filtering
    threshold = '',

    ##
    # Path to reference file for comparing similarity
    doc = '',
...
topics, model, similarity_base, threshold = None, None, None, None
...
def preprocessText(sentence):
    ...
    for word in sentence_words:
        if word in punctuations or word in stop_words:
            sentence_words.remove(word)
    return [wordnet_lemmatizer.lemmatize(word) for word in sentence_words if re.search("[A-Za-z]", word)]

def simDoc(text):
    f1 = preprocessText(text)
    global similarity_base, model
    sim = model.docvecs.similarity_unseen_docs(model, similarity_base, f1, steps=100)
    return sim
...
def extract_process(opts, i, jobs_queue, output_queue):
    ...
    while True:
        ...
        if job:
            ...
            output_queue.put((page_num, title, text))
            out.truncate(0)
            out.seek(0)
        else:
...
def checkTitle(title):
    modifiedTitle = " ".join(title.lower().split("_"))
    global topics
    if any([topic == modifiedTitle for topic in topics]):
        return True
    return False
...
def reduce_process(opts, output_queue, spool_length,
    ...
    while True:
        if next_page in spool:
            (title, text) = spool.pop(next_page)
            # print ("Getting to sim")
            similarity = simDoc(text)
            next_page += 1
            # tell mapper our load:
            spool_length.value = len(spool)
            # progress report
            # print ("here", similarity)
            if similarity < float(options.threshold) and not checkTitle(title):
                continue
            else:
            ...
            if not pair:
                break
            page_num, title, text = pair
            spool[page_num] = (title, text)
            # tell mapper our load:
            spool_length.value = len(spool)
